...
// src/data/tecnologias-autocompletado.json
var tecnologias_autocompletado_default = {
  _metadata: {
...
// src/core/catalog.ts
function getCatalog() {
  return tecnologias_autocompletado_default;
}
function getTechnologies() {
  return tecnologias_autocompletado_default.tecnologias;
}
function getMetadata() {
  return tecnologias_autocompletado_default._metadata;
}
function getTechnologyCount() {
  return tecnologias_autocompletado_default.tecnologias.length;
}
...
function isSimpleTechnology(tech) {
  return tech.tipo !== "Stack";
}

// src/core/getByType.ts
function getTechsByType(type) {
  const technologies = getTechnologies();
  return technologies.filter((tech) => tech.tipo === type);
}
function getTechsByTypes(types) {
  const technologies = getTechnologies();
  return technologies.filter((tech) => types.includes(tech.tipo));
}
function getSimpleTechnologies() {
  const technologies = getTechnologies();
  return technologies.filter((tech) => !isStackTechnology(tech));
}
function getStacks() {
  const technologies = getTechnologies();
  return technologies.filter((tech) => isStackTechnology(tech));
...
function getTechByNameStrict(name) {
  const technologies = getTechnologies();
  return technologies.find((tech) => tech.nombre === name.trim());
}
function getTechsByPartialName(partialName, caseSensitive = false) {
  const technologies = getTechnologies();
...
function techExists(name) {
  return getTechByName(name) !== void 0;
}
...
function getBestMatches(searchTerm, items, getItemString, maxResults = 10, threshold = 0.3) {
  const matches = items.map((item) => ({
    item,
    score: calculateSimilarity(searchTerm, getItemString(item))
  })).filter((match) => match.score >= threshold).sort((a, b) => b.score - a.score).slice(0, maxResults);
  return matches;
}

// src/search/search.ts
function searchTech(query, options = {}) {
  const {
    fuzzy = true,
    caseSensitive = false,
    maxResults = 20,
  ...
  if (categories && categories.length > 0) {
    technologies = technologies.filter(
      (tech) => categories.includes(tech.tipo)
    );
  }
  ...
  const exactMatches = technologies.filter((tech) => {
    const techName = caseSensitive ? tech.nombre : tech.nombre.toLowerCase();
...
function searchByName(name, maxResults = 10) {
  const technologies = getTechnologies();
  const matches = getBestMatches(
    name.toLowerCase(),
    technologies,
...
function autocomplete(input, maxSuggestions = 10, filterByType) {
...
function filterTechnologies(criteria) {
  let technologies = getTechnologies();
  if (criteria.types && criteria.types.length > 0) {
    technologies = technologies.filter(
      (tech) => criteria.types.includes(tech.tipo)
    );
  }
  if (criteria.nameContains) {
    const searchTerm = criteria.caseSensitive ? criteria.nameContains : criteria.nameContains.toLowerCase();
    technologies = technologies.filter((tech) => {
      const techName = criteria.caseSensitive ? tech.nombre : tech.nombre.toLowerCase();
      return techName.includes(searchTerm);
    });
  }
...
function getStacksByComponent(componentName) {
  const technologies = getTechnologies();
...
function getStacksByComponentType(componentType) {
  const technologies = getTechnologies();
  return technologies.filter((tech) => {
    if (!isStackTechnology(tech)) return false;
...
function getTechnologiesUsedInStacks() {
  const technologies = getTechnologies();
  const stackComponentNames = /* @__PURE__ */ new Set();
  technologies.forEach((tech) => {
  ...
  return technologies.filter(
    (tech) => stackComponentNames.has(tech.nombre.toLowerCase())
  );
}
function getStandaloneTechnologies() {
  const technologies = getTechnologies();
  const stackComponentNames = /* @__PURE__ */ new Set();
  technologies.forEach((tech) => {
    if (isStackTechnology(tech)) {
  ...
  return technologies.filter(
    (tech) => !isStackTechnology(tech) && !stackComponentNames.has(tech.nombre.toLowerCase())
  );
}
function sortByName(technologies, ascending = true) {
  return [...technologies].sort((a, b) => {
    const comparison = a.nombre.localeCompare(b.nombre);
    return ascending ? comparison : -comparison;
  });
}
function sortByType(technologies, typeOrder) {
  const defaultOrder = [
    "Language",
    "Framework",
    "Library",
 ...
 *
 * A comprehensive catalog of 1000+ technologies organized by categories,
 * with advanced search, filtering, and validation capabilities.
 *
 * @author SPARRING
 * @version 1.0.0
 * @license MIT
...
exports.catalog = tecnologias_autocompletado_default;
exports.filterTechnologies = filterTechnologies;
exports.getCatalog = getCatalog;
exports.getCategories = getCategories;
exports.getCountByCategory = getCountByCategory;
exports.getMetadata = getMetadata;
exports.getMostPopularCategory = getMostPopularCategory;
exports.getSimpleTechnologies = getSimpleTechnologies;
exports.getStacks = getStacks;
exports.getStacksByComponent = getStacksByComponent;
exports.getStacksByComponentType = getStacksByComponentType;
exports.getStandaloneTechnologies = getStandaloneTechnologies;
exports.getStatistics = getStatistics;
exports.getTechByName = getTechByName;
exports.getTechByNameStrict = getTechByNameStrict;
exports.getTechnologies = getTechnologies;
exports.getTechnologiesUsedInStacks = getTechnologiesUsedInStacks;
exports.getTechnologyCount = getTechnologyCount;
exports.getTechsByPartialName = getTechsByPartialName;
exports.getTechsByType = getTechsByType;
exports.getTechsByTypes = getTechsByTypes;
exports.isSimpleTechnology = isSimpleTechnology;
exports.isStackTechnology = isStackTechnology;
exports.isValidTechnologyType = isValidTechnologyType;
exports.sanitizeName = sanitizeName;
exports.searchByName = searchByName;
exports.searchTech = searchTech;
exports.sortByName = sortByName;
exports.sortByType = sortByType;
exports.techExists = techExists;
